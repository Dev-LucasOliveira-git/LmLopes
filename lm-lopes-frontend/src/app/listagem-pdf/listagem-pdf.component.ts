...
export class ListagemPdfComponent implements OnInit, AfterViewInit {
  ...
  async loadLogoImage(src: string): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
      const img = new Image();
      img.src = src;
      img.onload = () => resolve(img); // Resolve a Promise quando a imagem termina de carregar
      img.onerror = (error) => reject(error); // Rejeita se houver erro ao carregar
    });
  }
  ...
  async onRowClicked(pdf: any) {
    await this.loadImage(pdf.idOrdem)
    const doc = new jsPDF();
    const img = await this.loadLogoImage('assets/logo/logo-lm.png');
